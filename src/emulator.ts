...
export const DEV_CERT_PRIVATE_KEY =
...
export default class EmuContainer {
  ...
  constructor(
      elfLocalPath: string, libElfs: Record<string, string>, image: string,
      name: string) {
    this.image = image;
  ...
  static killContainerByName(name: string): void {
    ...
    docker.listContainers(
        ...
        (listError, containers?: ContainerInfo[]) => {
  ...
  static async checkAndPullImage(imageName: string): Promise<void> {
    const docker = new Docker();
    const ghcrImage =
        imageName.includes('ghcr.io') ? imageName : `ghcr.io/${imageName}`;

    await new Promise<void>((resolve) => {
      docker.pull(ghcrImage, {}, (err: any, stream: any) => {
        function onProgress(event: any): void {
          const progress = event?.progress ?? '';
          const status = event?.status ?? '';
          process.stdout.write(`[GHCR] ${status}: ${progress}\n`);
        }

        function onFinished(err: any, _output: any): void {
          if (err != null) {
            process.stdout.write(`[GHCR] ${err}\n`);
            throw err;
          } else {
            resolve();
          }
  ...
  async runContainer(options: {
    logging: boolean; custom: string; model: string; transportPort: string;
    ...
    const command =
        `/home/zondax/speculos/speculos.py --log-level speculos:DEBUG --color JADE_GREEN ${
            displaySetting} ${customOptions} -m ${modelOptions} ${
            DEFAULT_APP_PATH}/${appFilename} ${libArgs}`;
    ...
    const displayEnvironment: string = process.platform === 'darwin' ?
